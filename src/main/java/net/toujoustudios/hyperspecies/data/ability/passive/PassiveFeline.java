...
public class PassiveFeline extends PassiveAbility {
    ...
    public void execute(Player player) {
        ...
        if(player.getLocation().getBlock().getType() == Material.WATER) {
            player.addPotionEffect(new PotionEffect(PotionEffectType.SLOW_DIGGING, 20 * 10, 99, false, false, true));
        }

        if(player.getLocation().add(0, 2, 0).getBlock().getType() != Material.AIR) {
            player.addPotionEffect(new PotionEffect(PotionEffectType.SLOW, 20 * 5, 3, false, false, true));
            player.addPotionEffect(new PotionEffect(PotionEffectType.WEAKNESS, 20 * 5, 1, false, false, true));
        } else {
